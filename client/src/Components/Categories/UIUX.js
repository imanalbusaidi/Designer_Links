...
const UIUX = () => {
  ...
  useEffect(() => {
    if (!user) return; // Ensure user exists before fetching posts

    const fetchPosts = async () => {
      try {
        const response = await axios.get(`${SERVER_URL}/getPostsByCategory`);
        console.log("UI/UX posts response:", response.data); // Debugging log
        const categoryPosts = response.data.postsByCategory.find(
          (category) => category._id === "UI/UX"
        );
        console.log("Filtered UI/UX posts:", categoryPosts); // Debugging log
        ...
        console.error("Error fetching UI/UX posts:", error);
  ...
  const handlePost = async () => {
    ...
    formData.append("category", "UI/UX");
    if (image) {
  ...
  const filteredPosts = posts.filter(
    (post) => !post.postMsg.startsWith("This is a post about UI/UX by")
  );

  // Style objects to match other category pages
  const postStyle = {
    background: "#fff",
    borderRadius: "18px",
    ...
    <div className="category-page">
      ...
      {user.userType === "designer" && (
        <div className="share-post">
          ...
          <button
            style={buttonStyle}
            onMouseOver={(e) => (e.target.style.backgroundColor = buttonHoverStyle.background)}
